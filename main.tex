...
\makeatletter
% Compact Metropolis title page (prevents vertical overflow)
...
% TITLE INFORMATION
% ------------------------------------------------------------
\title{Industrial Maintenance}

\subtitle{
UNIT--IV: WORK STUDY AND\\
STATISTICAL QUALITY CONTROL
}

\author{Rachakatla Narmada}
...
% ------------------------------------------------------------
% CUSTOM FOOTER
% Left: author | Center: topic | Right: slide number
% Same dark color as header
% ------------------------------------------------------------
...
\defbeamertemplate{footline}{custom}{%
  ...
  \begin{beamercolorbox}[
    ...
    \hbox to \paperwidth{%
      % LEFT: AUTHOR
      \hbox to 0.30\paperwidth{%
        \hspace{0.40cm}%
        \scriptsize\color{white}\insertshortauthor
        \hfil
      }%
      % CENTER: TOPIC
      \hbox to 0.50\paperwidth{%
        \hfil
        \scriptsize\color{white}\textbf{Industrial Maintenance}%
        \hfil
      }%
      % RIGHT: SLIDE NUMBER
      \hbox to 0.20\paperwidth{%
        \hfil
        \scriptsize\color{white}\insertframenumber
        \hspace{0.40cm}%
      }%
    }%
  \end{beamercolorbox}%
...
\begin{frame}[shrink=18]{What is Work Study?}
\small
...
\begin{frame}[shrink=18]{Why is Work Study Required?}
\small
...
\begin{frame}[shrink=18]{Objectives of Work Study}
\small
...
\begin{frame}[shrink=18]{Work Study: Basic Procedure}
\small
...
\begin{frame}[shrink=18]{Method Study}
\small
...
\begin{frame}[shrink=18]{Method Study Procedure}
\small
...
\begin{frame}[shrink=18]{SREDIM Technique}
\small
...
\begin{frame}[shrink=18]{Critical Examination Questions}
\small
...
\begin{frame}[shrink=18]{ECRS Principle}
\small
...
\begin{frame}[shrink=18]{Purpose of Method Study Charts}
\small
...
\begin{frame}[shrink=18]{Operation Process Chart}
\small
...
\small
A \textbf{Flow Process Chart} gives a detailed representation of the process.
It normally identifies \textbf{operation, inspection, transport, delay,} and \textbf{storage}.
\vspace{2pt}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\textbf{Typical analysis}
\begin{itemize}
\item How many operations?
\item How much distance is travelled?
\end{itemize}
\column{0.48\textwidth}
\begin{itemize}
\item How much waiting occurs?
\item How many storage points exist?
\end{itemize}
\end{columns}
\vspace{3pt}
...
\begin{frame}[shrink=18]{Two-Handed Process Chart}
\small
...
\begin{frame}[shrink=18]{Man--Machine Chart}
\small
...
\begin{frame}[shrink=18]{Comparison of Method Study Charts}
\small
...
\begin{frame}[shrink=18]{What is Work Measurement?}
\small
...
\begin{frame}[shrink=18]{Major Work Measurement Techniques}
\small
...
\begin{frame}[shrink=18]{Time Study: Basic Concept}
\small
...
\begin{frame}[shrink=18]{Stopwatch Time Study}
\small
...
\begin{frame}[shrink=18]{Why Divide a Job into Elements?}
\small
...
\begin{frame}[shrink=18]{Continuous Timing}
\small
...
\begin{frame}[shrink=18]{Snapback / Flyback Timing}
\small
...
\begin{frame}[shrink=18]{Observed Time}
\small
...
\begin{frame}[shrink=18]{Abnormal Observations}
\small
...
\begin{frame}[shrink=18]{Why is Performance Rating Needed?}
\small
...
\begin{frame}[shrink=18]{Physical Meaning of Rating}
\small
...
\begin{frame}[shrink=18]{Rating Less Than 100\%}
\small
...
\begin{frame}[shrink=18]{Rating Methods}
\small
...
\begin{frame}[shrink=18]{Normal Time}
\small
...
\begin{frame}[shrink=18]{Numerical Example: Normal Time}
\small
...
\begin{frame}[shrink=18]{Why are Allowances Required?}
\small
...
\begin{frame}[shrink=18]{Types of Allowances}
\small
...
\begin{frame}[shrink=18]{Standard Time: Allowance Convention 1}
\small
...
\begin{frame}[shrink=18]{Standard Time: Allowance Convention 2}
\small
...
\begin{frame}[shrink=18]{Why Two Allowance Formulas Exist}
\small
...
\small
Suppose $OT=2.0\,\mathrm{min}$, $R=110\%$, and $A=15\%$ (allowance as a fraction of normal time).
\vspace{2pt}
\begin{align*}
NT&=OT\left(\frac{R}{100}\right)=2.0(1.10)=2.20\,\mathrm{min},\\
ST&=NT(1+A)=2.20(1.15)=2.53\,\mathrm{min}.
\end{align*}
\begin{center}
\boxed{OT\;\rightarrow\;NT\;\rightarrow\;ST}
\qquad
\boxed{2.00\;\rightarrow\;2.20\;\rightarrow\;2.53\ \mathrm{min}}
\end{center}
\end{frame}
...
\begin{frame}[shrink=18]{What is Work Sampling?}
\small
...
\begin{frame}[shrink=18]{When is Work Sampling Useful?}
\small
...
\[
X=\begin{cases}1,&\text{activity is observed}\\0,&\text{activity is not observed}\end{cases}
\]
\begin{align*}
P(X=1)&=p, & P(X=0)&=1-p=q,\\
...
E(X)&=np, & Var(X)&=np(1-p).
\end{align*}
The model assumes independent observations with a constant activity probability $p$.
\end{frame}

% ------------------------------------------------------------

\begin{frame}[shrink=18]{Estimator of Activity Proportion}
\small
...
\begin{frame}[shrink=18]{Variance and Standard Error}
\small
...
\begin{frame}[shrink=18]{Confidence Interval for Work Sampling}
\small
...
\begin{frame}[shrink=18]{Sample Size for Work Sampling}
\small
...
\small
Find $n$ for 95\% confidence, error $e=0.05$, assuming $p=0.50$.
\[
\begin{aligned}
n&=\frac{(1.96)^2(0.5)(0.5)}{(0.05)^2}\\
 &=\frac{3.8416(0.25)}{0.0025}=384.16.
\end{aligned}
\]
\begin{center}\boxed{n=385\ \text{observations}}\end{center}
...
\small
Suppose $n=500$ random observations are taken and the worker is idle in $x=75$ observations.
\begin{align*}
\hat p&=\frac{75}{500}=0.15=15\%,\\
SE&=\sqrt{\frac{0.15(0.85)}{500}}\approx0.01597.
\end{align*}
Thus approximately 15\% of observations indicate idle time.
\end{frame}

% ------------------------------------------------------------

\begin{frame}[shrink=18]{Interpretation of Work Sampling Result}
\small
...
\begin{frame}[shrink=18]{What is Statistical Quality Control?}
\small
...
\begin{frame}[shrink=18]{Why Does Variation Occur?}
\small
...
\begin{frame}[shrink=18]{Common Causes}
\small
...
\begin{frame}[shrink=18]{Special Causes}
\small
...
\begin{frame}[shrink=18]{Control Limits vs Specification Limits}
\small
...
\begin{frame}[shrink=18]{Variables Data}
\small
...
\begin{frame}[shrink=18]{Attributes Data}
\small
...
\begin{frame}[shrink=18]{Variables vs Attributes}
\small
...
\begin{frame}[shrink=18]{Basic Structure of a Control Chart}
\small
...
\begin{frame}[shrink=18]{Why Three Sigma?}
\small
...
\begin{frame}[shrink=18]{Interpretation of a Control Chart}
\small
...
\begin{frame}[shrink=18]{$\bar X$--$R$ Control Charts}
\small
...
\begin{frame}[shrink=18]{$\bar X$ Chart}
\small
...
\begin{frame}[shrink=18]{Why Rational Subgroups?}
\small
...
\begin{frame}[shrink=18]{Important $\bar X$--$R$ Constants}
\small
...
\begin{frame}[shrink=18]{Estimating Process Standard Deviation}
\small
...
\begin{frame}[shrink=18]{$\bar X$--$R$: Numerical Example}
\small
...
\begin{frame}[shrink=18]{$\bar X$--$R$: Control Limits}
\small
...
\begin{frame}[shrink=18]{R Chart: Numerical Calculation}
\small
...
\begin{frame}[shrink=18]{Defect vs Defective}
\small
...
\begin{frame}[shrink=18]{$p$ Control Chart}
\small
...
\begin{frame}[shrink=18]{Mean Fraction Defective}
\small
...
\begin{frame}[shrink=18]{$p$ Chart Control Limits}
\small
...
\small
Suppose $n=100$ units are inspected in each sample, with defectives $8,5,7,10,6$.
\[
\sum d_i=36,\qquad \sum n_i=5(100)=500.
\]
Therefore:
\[
\boxed{\bar p=\frac{36}{500}=0.072=7.2\%}
\]
\end{frame}
...
\small
For $\bar p=0.072$ and $n=100$:
\begin{align*}
\sigma_p&=\sqrt{\frac{0.072(0.928)}{100}}\approx0.02585,\\
UCL&=0.072+3(0.02585)\approx0.1496,\\
LCL&=0.072-3(0.02585)<0\;\Rightarrow\;LCL=0.
\end{align*}
\boxed{UCL\approx0.1496,\qquad LCL=0}
\end{frame}

% ------------------------------------------------------------

\begin{frame}[shrink=18]{Unequal Sample Sizes in a $p$ Chart}
\small
...
\begin{frame}[shrink=18]{$c$ Control Chart}
\small
...
\begin{frame}[shrink=18]{$c$ Chart Control Limits}
\small
...
\begin{frame}[shrink=18]{$c$ Chart: Numerical Example}
\small
...
\begin{frame}[shrink=18]{$p$ Chart vs $c$ Chart}
\small
...
\begin{frame}[shrink=18]{What is Acceptance Sampling?}
\small
...
\begin{frame}[shrink=18]{Why Use Acceptance Sampling?}
\small
...
\begin{frame}[shrink=18]{Single Sampling Plan}
\small
...
\begin{frame}[shrink=18]{Binomial Model for Acceptance Sampling}
\small
...
\small
Consider $n=20$, $c=1$, and $p=0.05$. The lot is accepted when $X=0$ or $X=1$.
\begin{align*}
P_a&=P(X=0)+P(X=1)\\
&=(0.95)^{20}+20(0.05)(0.95)^{19}.
\end{align*}
For the binomial model, this is the probability that the sample contains no more than one defective unit.
\end{frame}

% ------------------------------------------------------------

\begin{frame}[shrink=18]{Numerical Acceptance Probability}
\small
...
\begin{frame}[shrink=18]{Hypergeometric Model}
\small
...
\begin{frame}[shrink=18]{Double Sampling Plan}
\small
...
\begin{frame}[shrink=18]{Conceptual Double Sampling Example}
\small
...
\begin{frame}[shrink=18]{Operating Characteristic Curve}
\small
...
\begin{frame}[shrink=18]{Shape of an OC Curve}
\small
...
\begin{frame}[shrink=18]{Effect of Acceptance Number \(c\)}
\small
...
\begin{frame}[shrink=18]{Effect of Sample Size \(n\)}
\small
...
\begin{frame}[shrink=18]{Acceptable Quality Level}
\small
...
\begin{frame}[shrink=18]{LTPD / RQL}
\small
...
\begin{frame}[shrink=18]{Producer's Risk \(\alpha\)}
\small
...
\begin{frame}[shrink=18]{Consumer's Risk \(\beta\)}
\small
...
\begin{frame}[shrink=18]{Producer vs Consumer Risk}
\small
...
\begin{frame}[shrink=18]{Process Control vs Lot Acceptance}
\small
...
\begin{frame}[shrink=18]{Assumptions in Time Study}
\small
...
\begin{frame}[shrink=18]{Limitations of Time Study}
\small
...
\begin{frame}[shrink=18]{Assumptions in Work Sampling}
\small
...
\begin{frame}[shrink=18]{Assumptions in Control Charts}
\small
...
\begin{frame}[shrink=18]{Integrated Manufacturing Scenario}
\small
...
\begin{frame}[shrink=18]{Integrated Method Study}
\small
...
\begin{frame}[shrink=18]{Integrated Time Study}
\small
...
\begin{frame}[shrink=18]{Capacity Interpretation}
\small
...
\begin{frame}[shrink=18]{Integrated Quality Monitoring}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: Time Study}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: Work Sampling}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: $\bar X$--$R$}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: R Chart}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: $p$ Chart}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: $c$ Chart}
\small
...
\begin{frame}[shrink=18]{Formula Sheet: Acceptance Sampling}
\small
...
\begin{frame}[shrink=18]{Method Study vs Work Measurement}
\small
...
\begin{frame}[shrink=18]{Time Study vs Work Sampling}
\small
...
\begin{frame}[shrink=18]{Control Charts Comparison}
\small
...
\begin{frame}[shrink=18]{Important Distinctions}
\small
...
\begin{frame}[shrink=18]{Viva Questions: Work Study}
\small
...
\begin{frame}[shrink=18]{Viva Questions: Time Study}
\small
...
\begin{frame}[shrink=18]{Viva Questions: Work Sampling}
\small
...
\begin{frame}[shrink=18]{Viva Questions: SQC}
\small
...
\begin{frame}[shrink=18]{Viva Questions: Control Charts}
\small
...
\begin{frame}[shrink=18]{Viva Questions: Acceptance Sampling}
\small
...
\small
Given $OT=4.5\,\mathrm{min}$, $R=110\%$, and $A=20\%$ of normal time.
\textbf{Find:} normal time and standard time.
\begin{align*}
NT&=4.5(1.10)=4.95\,\mathrm{min},\\
ST&=4.95(1.20)=5.94\,\mathrm{min}.
\end{align*}
\begin{center}\boxed{ST=5.94\,\mathrm{min}}\end{center}
\end{frame}

% ------------------------------------------------------------

\begin{frame}[shrink=18]{Numerical Problem 2: Work Sampling}
\small
...
\begin{frame}[shrink=18]{Numerical Problem 3: $\bar X$ Chart}
\small
...
\small
Suppose $\bar p=0.04$ and $n=200$.
\begin{align*}
\sigma_p&=\sqrt{\frac{0.04(0.96)}{200}}\approx0.01386,\\
UCL&=0.04+3(0.01386)\approx0.0816,\\
LCL&=0.04-3(0.01386)<0\;\Rightarrow\;LCL=0.
\end{align*}
\boxed{UCL\approx0.0816,\qquad LCL=0}
\end{frame}

% ------------------------------------------------------------

\begin{frame}[shrink=18]{Numerical Problem 5: $c$ Chart}
\small
...
\begin{frame}[shrink=18]{Physical Meaning of Standard Time}
\small
...
\begin{frame}[shrink=18]{Physical Meaning of Control Limits}
\small
...
\begin{frame}[shrink=18]{Physical Meaning of Process Capability}
\small
...
\begin{frame}[shrink=18]{Why Statistical Thinking Matters}
\small
...
\begin{frame}[shrink=18]{Complete Unit--IV Concept Map}
\small
...
\]
\[
\boxed{UCL_{\bar X}=\bar{\bar X}+A_2\bar R}
\]
\column{0.48\textwidth}
\[
\boxed{LCL_{\bar X}=\bar{\bar X}-A_2\bar R}
\]
\[
\boxed{UCL_R=D_4\bar R}
\]
\[
\boxed{UCL_p=\bar p+3\sqrt{\frac{\bar p(1-\bar p)}{n}}}
\]
\[
\boxed{UCL_c=\bar c+3\sqrt{\bar c}}
\]
\end{columns}
\end{frame}
...
\column{0.48\textwidth}
\begin{itemize}
\item Work study and objectives
...
\column{0.48\textwidth}
\begin{center}
\textbf{1. Improve the Method}\\[3pt]
$\downarrow$\\[3pt]
\textbf{2. Measure the Work}\\[3pt]
$\downarrow$\\[3pt]
\textbf{3. Establish Normal and Standard Time}
\end{center}
\column{0.48\textwidth}
\begin{center}
\textbf{4. Monitor Process Variation}\\[3pt]
$\downarrow$\\[3pt]
\textbf{5. Identify Statistical Signals}\\[3pt]
$\downarrow$\\[3pt]
\textbf{6. Investigate and Improve the Process}
\end{center}
\end{columns}
\vspace{6pt}
\begin{center}
\boxed{\text{Productivity improvement and quality control are continuous engineering activities}}
\end{center}
\end{frame}
